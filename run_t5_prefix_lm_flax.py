...
from modeling_t5_flax import FlaxT5ForConditionalGeneration
...
class ModelArguments:
    ...
    layer_norm_type: Optional[str] = field(
        default=None, metadata={"help": "Type of LayerNorm to use for T5; e.g: vanilla (defaults to bias-less version)"}
    )
...
class DataTrainingArguments:
    ...
    min_prefix_length: Optional[int] = field(
        default=13,
        metadata={
            "help": "The minimum length of the prefix, as well as the minimum length of the target span"
        },
    )
...
class FlaxDataCollatorForT5PrefixLM:
    """
    ...
    Args:
        ...
        input_length (:obj:`int`):
            The maximum input length after masking.
        min_prefix_length (:obj:`int`):
            The minimum possible length of the prefix, and also the minimum possible length of the target span to predict
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
        ...
        assert expanded_input_length == self.input_length + self.min_prefix_length
        ...
        if batch["input_ids"].shape[-1] > self.input_length:
            raise ValueError(
                f"`input_ids` are incorrectly preprocessed. `input_ids` length is {batch['input_ids'].shape[-1]}, "
                f"but should be less than or equal to {self.input_length}."
            )

        if batch["labels"].shape[-1] > self.input_length:
            raise ValueError(
                f"`labels` are incorrectly preprocessed. `labels` length is {batch['labels'].shape[-1]}, "
                f"but should be less than or equal to {self.input_length}."
            )
...
def main():
    ...
    expanded_inputs_length = max_seq_length + data_args.min_prefix_length
    ...
    data_collator = FlaxDataCollatorForT5PrefixLM(
        tokenizer=tokenizer,
        input_length=max_seq_length,
        min_prefix_length=data_args.min_prefix_length,
    ...
    def eval_step(params, batch):
        ...
        # summarize metrics
        metrics = {"loss": loss.mean(), "specialization": specialization}
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                epochs.write(f"Step... ({cur_step} | "
                             ...
                             f"Perplexity: {eval_metrics['perplexity']},"
