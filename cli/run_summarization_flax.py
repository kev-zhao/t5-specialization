...
def main():
    ...
    if training_args.do_train:
        ...
        train_dataset = train_dataset.map(lambda batch: pd.DataFrame(batch["translation"]).to_dict(orient="list"),
                                          batched=True, remove_columns=["translation"])
    ...
    if training_args.do_eval:
        ...
        eval_dataset = eval_dataset.map(lambda batch: pd.DataFrame(batch["translation"]).to_dict(orient="list"),
                                          batched=True, remove_columns=["translation"])
    ...
    if training_args.do_predict:
        ...
        predict_dataset = predict_dataset.map(lambda batch: pd.DataFrame(batch["translation"]).to_dict(orient="list"),
                                        batched=True, remove_columns=["translation"])
    ...
    def train_step(state, batch, label_smoothing_factor=0.0):
        ...
        metrics = {"loss": loss, "learning_rate": linear_decay_lr_schedule_fn(state.step), "specialization": compute_specialization_metric(encoder_attns)}
    ...
    def eval_step(params, batch, label_smoothing_factor=0.0):
        ...
        # summarize metrics
        metrics = {"loss": loss, "specialization": compute_specialization_metric(jnp.swapaxes(jnp.stack(outputs["encoder_attentions"]), 0, 1))}
        metrics = jax.lax.pmean(metrics, axis_name="batch")
        return metrics
        ...
        for step in tqdm(range(steps_per_epoch), desc="Training...", position=1, leave=False):
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                epochs.write(f"Step... ({cur_step} | "
                             ...
                             f"SacreBLEU: {eval_metrics['score']},"
